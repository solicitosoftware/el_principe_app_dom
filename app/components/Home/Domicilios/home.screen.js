...
import {RUTA_FUNCTIONS} from '../../../redux/config';
...
function Home({route}) {
  ...
  useEffect(() => {
    const cargarUsuario = async () => {
      ...
      const doc = await firebase.db.collection('empleados').doc(id).get();
      setDomiciliario({
        id: doc.id,
        ...doc.data(),
      });
  ...
  useEffect(() => {
    const obtenerDeudas = () => {
      firebase.db
        ...
        .where('domiciliario.id', '==', domiciliario.id)
    ...
    //Metodos para obtener los pedidos del domiciliario
  ...
  function manejarSnapshotPedidos(values) {
  ...
  function manejarSnapshotDeudas(values) {
  ...
  const cargarDetalle = () => {
    ...
    return (
      <>
        ...
        <View style={{marginVertical: normalize(15, 'height')}}>
          ...
          {cliente && cliente.telefono2 && (
            <View
              ...
              <Text style={styles.text}>
                <Text style={{fontWeight: 'bold'}}>Teléfono Secundario: </Text>
                {cliente.telefono2}
              </Text>
              <Icon
                reverse
            ...
            {detallePago
              ? medioPago !== 'efectivo' &&
                detallePago.transferencia > 0 && (
                  <Text style={styles.text}>
                    <Text style={{fontWeight: 'bold'}}>TRANSFERENCIA: </Text>
          ...
          {comentario ? (
            <View
              ...
              <Text style={styles.text}>
                <Text style={{fontWeight: 'bold'}}>Motivo Cancelación: </Text>
                {comentario}
              </Text>
            </View>
          ) : null}
        ...
        {habilitarBoton && (
          <TouchableNativeFeedback onPress={() => actualizarPedido(id)}>
            <Button
              style={styles.button}
              theme={{
                colors: {
